...
export class AsignarTransporteComponent implements OnInit {
	...
	seleccionarProveedorTransporte(event: Event): void {
		...
		if (!id) return;
		const p = this.proveedoresTransporte.find(x => x.id === id);
		if (!p) return;
	...
	cerrarForm(): void { this.showForm = false; this.viajeSeleccionado = null; this.editandoTransporteId = null; this.showCalendarHorario = false; }
