...
export class DashboardOperativoComponent implements OnInit {
  cargando = false;
  filas: FilaDashboard[] = [];
  paquetes: { id: number; titulo: string; destino: string }[] = [];
  ...
  eliminarViajeRow(fila: FilaDashboard): void {
    if (!fila.idViaje) return;
    if (!confirm(`¿Eliminar el viaje #${fila.idViaje} del paquete "${fila.paqueteTitulo}"? Esta acción no se puede deshacer.`)) return;

    this.svc.eliminarViaje(fila.idViaje).subscribe({
      next: () => {
        this.mostrarToast('Viaje eliminado');
        this.cargar();
      },
      error: (err) => {
