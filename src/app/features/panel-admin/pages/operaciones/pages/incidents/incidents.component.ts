...
export class IncidentesComponent implements OnInit {
  ...
  incEstadoOpen: number | null = null;
  ...
  cerrarEstadoDropdown() { this.incEstadoOpen = null; }
  enviando = false;
  showToast = false;
  toastMsg = '';
  ...
  toggleEstadoDropdown(incId: number, event: Event): void {
    event.stopPropagation();
    this.incEstadoOpen = this.incEstadoOpen === incId ? null : incId;
  }

  seleccionarEstado(inc: Incidente, estado: string, event: Event): void {
    event.stopPropagation();
    this.incEstadoOpen = null;
    if (inc.estado === estado) return;
    this.svc.actualizarEstadoIncidente(inc.id, estado).subscribe({
      next: (result) => {
